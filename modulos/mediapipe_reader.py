...
face_mesh = mp_face_mesh.FaceMesh(
    max_num_faces=1,
    refine_landmarks=True, # Vital para medir el cierre de los párpados
...
def distancia(p1, p2, w, h):
    """Calcula la distancia en píxeles entre dos puntos faciales"""
...
def generador_mediapipe():
    ...
    while cap.isOpened():
        ...
        # Volver a BGR para mostrarla en ventana
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                # 1. SOLUCIÓN VERTICAL: Invertimos el Pitch con un signo negativo
                hx = yaw / 100.0   
                hy = -pitch / 100.0 
                
                img_h, img_w, _ = image.shape
                
                # 2. SOLUCIÓN SENSIBILIDAD DE BOCA: Multiplicador x4
                dist_boca = distancia(face_landmarks.landmark[13], face_landmarks.landmark[14], img_w, img_h)
                y_57 = dist_boca * 4.0 
                
                # 3. SOLUCIÓN PAUSA (OJOS CERRADOS): Algoritmo Eye Aspect Ratio (EAR)
                # Ojo izquierdo (Vertical / Horizontal)
                ...
                # Ojo derecho (Vertical / Horizontal)
                ...
                # Si ambos ojos caen por debajo de un ratio de 0.22, se consideran cerrados
                if ear_izq < 0.22 and ear_der < 0.22:
                    au45 = 1.0 # Enviar orden de pausa al Sillodromo
                else:
                    au45 = 0.0
                
                conf = 0.98

                # Dibujar malla verde para que puedas evaluar el tracking y la latencia
        ...
        # 4. SOLUCIÓN VENTANA DE CÁMARA
        cv2.imshow("Vision Inteligente - Sillodromo", image)
        cv2.waitKey(1) # Refresca la ventana en tiempo real
