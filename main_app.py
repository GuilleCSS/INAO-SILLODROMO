...
from modulos.camara import HiloCamara
...
class HiloProcesamiento(QThread):
    ...
    def __init__(self):
        ...
        self.proceso_of = None
        self._activo = True

    def run(self):
        try:
            self.senal_mensaje.emit("Inicializando cámara y modelos de OpenFace…")
            ...
            for datos in stream_openface_csv(csv_file):
                if not self._activo:
                    break
                estado = self.controller.process_frame(*datos)
                if estado:
...
class ControlCentral(QMainWindow):
    ...
    def __init__(self):
        ...
        self.hilo.senal_listo.connect(self._iniciar_camara)
    ...
    def _iniciar_camara(self):
        if not config.get("vista_camara", True):
            ...
            return
        self.lblInfoCamara.setText("Conectando vista…")
        self.hilo_camara = HiloCamara()
        self.hilo_camara.senal_frame.connect(self._frame_camara)
        self.hilo_camara.senal_no_disponible.connect(self._sin_video)
